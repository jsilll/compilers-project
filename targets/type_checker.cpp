...
void l22::type_checker::do_declaration_node(l22::declaration_node *node, int lvl)
{
  ...
  else
  {
    if (node->initializer())
    {
      ...
      {
        // TODO: condição tem que ser mais forte que isto?
...
void l22::type_checker::do_assignment_node(cdk::assignment_node *const node, int lvl)
{
  ...
  if (node->rvalue()->is_typed(cdk::TYPE_UNSPEC))
  {
    ...
    else if (stack != nullptr)
    {
      ...
      {
        std::cout << std::string("THROW: A pointer is required to allocate.") << std::endl;
...
void l22::type_checker::do_function_call_node(l22::function_call_node *node, int lvl)
{
  ...
  if (node->arguments())
  {
    ...
    for (size_t i = 0; i < fType->input()->size(); i++)
    {
      if (!(node->argument(i)->is_typed(fType->input(i)->name()) || (node->argument(i)->is_typed(cdk::TYPE_INT) && (fType->input(i)->name() == cdk::TYPE_DOUBLE))))
      {
